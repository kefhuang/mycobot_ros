...
class Window:
    def __init__(self, handle):
        ...
        tk.Button(self.frmLT, text="Set Joint Angle", width=10, command=self.get_joint_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )

        # coordination settings button,coordination 设置按钮
        tk.Button(self.frmRT, text="Set Coordinate", width=10, command=self.get_coord_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )

        # Gripper switch button,夹爪开关按钮
        tk.Label(self.frmLB, text="Gripper").grid(row=1, padx=3, pady=(50, 2))
        tk.Button(self.frmLB, text="Open", command=self.gripper_open, width=5).grid(
            row=2, column=0, sticky="w", padx=3, pady=2
        )
        tk.Button(self.frmLB, text="Close", command=self.gripper_close, width=5).grid(
